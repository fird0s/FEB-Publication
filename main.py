...
def tambah_jurnal():
	...
	if request.method == "POST":
		...
		if request.form["jurnal-category"] and request.form["jurnal-name"] and request.form["jurnal-summary"] and request.form["JenisJurnal"]:
			JenisJurnal = Jenis_Jurnal.objects.get(nama=request.form["JenisJurnal"])
			get_db = Jurnal(author=user, jurnal_name=request.form["jurnal-name"], date_added=datetime.datetime.now(), kategori=request.form["jurnal-category"], summary=request.form["jurnal-summary"], reader_count = 1, jenis_jurnal=JenisJurnal)
			get_db.save()
	...
	return render_template("/admin/user-tambah-jurnal.html", user=user, get_all_cat=get_all_cat, get_all_jenis=get_all_jenis)

@elearning.route('/admin/edit/jurnal/<route>', methods=["GET", "POST"])
def edit_jurnal(route):
	...
	if request.method == "POST":
		if request.form["jurnal-name"] and request.form["jurnal-category"] and request.form["jurnal-summary"] and request.form["JenisJurnal"]:
			JenisJurnal = Jenis_Jurnal.objects.get(nama=request.form["JenisJurnal"])
			get_db.jenis_jurnal = JenisJurnal
	...
	return render_template("admin/user-edit-jurnal.html", user=user, get_db=get_db, get_all_cat=get_all_cat, get_all_jenis=get_all_jenis)
...
def view_jurnal(route):
	try:
		get_jurnal = Jurnal.objects.get(pk = ObjectId(route))
	except bson.errors.InvalidId:
		return redirect(url_for('index'))			
	return render_template("viewer.html", get_jurnal=get_jurnal)
	
@elearning.route("/view/summary/<route>")
def view_summary(route):
	try:
		get_jurnal = Jurnal.objects.get(pk = ObjectId(route))
		get_jurnal.reader_count = get_jurnal.reader_count + 1 
		get_jurnal.save()
	
		get_profile = Profile.objects.get(author=get_jurnal.author)
	except bson.errors.InvalidId:
		return redirect(url_for('index'))			
	return render_template("summary.html", get_jurnal=get_jurnal, get_profile=get_profile)	
